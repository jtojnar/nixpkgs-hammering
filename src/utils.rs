...
pub(crate) fn run_command_with_input(
    ...
    let mut child = Command::new(program)
        ...
        .stdout(Stdio::piped())
        .spawn()
        .map_err(|err| format!("Unable to spawn program ‘{program}’: {err}"))?;

    {
        ...
        child_stdin
            ...
            .map_err(|err| format!("Unable to write to stdin of ‘{program}’: {err}"))?;
    }

    let output = child
        .wait_with_output()
        .map_err(|err| format!("Unable to wait on ‘{program}’: {err}"))?;

    let stdout = String::from_utf8(output.stdout)
        .map_err(|err| format!("Unable to parse output of ‘{program}’ as UTF-8: {err}"))?;
