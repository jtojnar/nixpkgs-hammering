...
pub fn find_comment_within(node: &SyntaxNode) -> Option<String> {
    ...
    let doc = collect_comment_text(node_iter, true);
...
pub fn find_comment_after(node: &SyntaxNode) -> Option<String> {
    let node_iter = next_siblings(&NodeOrToken::Node(node.clone())).skip(1);
    let doc = collect_comment_text(node_iter, false);
    Some(doc).filter(|it| !it.is_empty())
}

fn collect_comment_text(
    node_iter: impl Iterator<Item = SyntaxElement>,
    allow_newline: bool,
) -> String {
    ...
    // If allow_newline = false, we skip over only non-newline containing
    // whitespace tokens
    node_iter
        .filter_map(|node| node.into_token())
        .take_while(|tok| {
            tok.kind() == TOKEN_COMMENT
                || (tok.kind().is_trivia() && (allow_newline || (!tok.text().contains("\n"))))
        })
