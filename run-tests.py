...
def make_test_rule(
    ...
    if matching_variants is not None:
        ...
        for variant in matching_variants:
            suite.addTest(
                FunctionTestCase(
                    make_test_variant(rule, variant, prebuild=prebuild),
                    description=f"{rule}.{variant}",
                )
            )
        ...
        for variant in nonmatching_variants:
            suite.addTest(
                FunctionTestCase(
                    make_test_variant(
                        rule, variant, should_match=False, prebuild=prebuild
                    ),
                    description=f"{rule}.{variant}",
                )
            )
...
class TestSuite(unittest.TestSuite):
    ...
    def __iter__(self):
        ...
        yield make_test_rule("fixup-phase")

        yield make_test_rule(
            "license-missing",
            ["no-license", "empty-license", "no-meta"],
            ["have-license"],
        )

        yield make_test_rule(
            "maintainers-missing",
            ["no-maintainers", "empty-maintainers", "no-meta"],
            [
                "have-maintainers",
            ],
        )

        yield make_test_rule("meson-cmake")
        ...
        yield make_test_rule("patch-phase")
